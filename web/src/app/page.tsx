...
export default function Home() {
  ...
  const autoFillColorChannels = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(channels.error ?? "Could not find compatible filter bands");
      setColorChannels(channels);
      setStatus("Suggested RGB bands loaded. Review them, then create the color PNG.");
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "Could not find compatible filter bands");
    } finally {
      setColorLoading(false);
    }
  };
